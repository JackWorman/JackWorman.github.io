...
function renderNeuralNetwork() {
  ...
  for (let i = 0; i < 28; i++) {
    for (let j = 0; j < 16; j++) {
      ...
      const intensity = evolutionaryAlgorithm.neuralNetworks[evolutionaryAlgorithm.specie].i.elements[i][0] * Math.abs(evolutionaryAlgorithm.neuralNetworks[evolutionaryAlgorithm.specie].w1.elements[j][i]);
  ...
  for (let i = 0; i < 16; i++) {
    for (let j = 0; j < 4; j++) {
      ...
      CONTEXT_NEURAL_NETWORK.closePath();
      const intensity = evolutionaryAlgorithm.neuralNetworks[evolutionaryAlgorithm.specie].hL.elements[i][0] * Math.abs(evolutionaryAlgorithm.neuralNetworks[evolutionaryAlgorithm.specie].w2.elements[j][i]);
