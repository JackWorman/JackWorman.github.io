...
export default class Sphere extends PhysicsObject {
    ...
    createFaces() {
        ...
        const spherePoint = (thetaInc, phiInc) => {
            const theta = (thetaInc / INC) * 2 * Math.PI;
            const phi = (phiInc / INC) * 2 * Math.PI;
            return new Vector3(this.radius * Math.sin(theta) * Math.cos(phi), this.radius * Math.cos(theta), this.radius * Math.sin(theta) * Math.sin(phi));
        };
        const faces = [];
